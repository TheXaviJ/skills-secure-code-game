...
def validorder(order: Order):
    ...
    for item in order.items:
        if item.type == 'payment':
            if item.amount > -1*MAX_ITEM_AMOUNT and item.amount < MAX_ITEM.AMOUNT:
                net += Decimal(str(item.amount))
        elif item.type == 'product':
            if item.quantity > 0 and item.quantity <= MAX_QUANTITY and item.amount > 0 and item.amount <= MAX_ITEM.AMOUNT:
